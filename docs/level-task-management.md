...
本次待发布的[小游戏擂台任务扩展](arena-level-task.md)继续使用原等级 API。仅 task80 增加安装能力、每日尝试和固定提示；当日禁用补挂不禁用次日自动计划设置。原 QQ 完成与加速数据保持原样。正式 API 已部署至授权测试服务 `mknt.bilibilibot.com`，1060221 已通过正式 worker 整局自然出分及等级入账验收，尚未发布新版本或部署生产环境。
