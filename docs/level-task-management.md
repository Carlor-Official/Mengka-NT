...
v2.3.0 的[小游戏擂台任务扩展](arena-level-task.md)继续使用原等级 API。task80 改为 Go 直接协议上报方块冲刺 v6 的 1–99 随机整数分数，不运行游戏、不需要外部 worker 安装。能力、每日尝试和固定提示字段不变；当日禁用补挂不禁用次日自动计划设置。只有独立 QQ 刷新确认完成才算成功。2026-09-15 02:37，2082083 已在 Linux amd64 测试站通过一次标准 API 的 Go 直接模式验收，依据本次独立证据，不沿用旧 worker 整局结论。升级时保留 journal，见[升级说明](arena-direct-score-upgrade.md)。
...
原 `get_level_tasks` 与 `execute_level_tasks` 保持原 action 和参数形式，新的完整面板接口使用独立 action 名。框架统一移除会员推广位及日常、额外分类中的付费任务；网页、缓存和插件面板只接收免费任务。框架注入的会员签到是免费领取任务，继续单列在“会员签到”分类。插件无需维护付费标记、进度推算或计划数据库副本。

所有已纳入框架执行器的 QQ 基础/额外加速项目，都通过截图所列的等级任务查询、计划和执行 API 暴露；插件不再维护单项目 action。
...
“听歌领金币兑换加速”不增加公开参数。v2.4.4 起，QQ 音乐余额查询结果不再作为执行前置条件，框架以兑换接口的业务结果判断成功或金币不足，避免部分账号余额接口返回 `0` 时被误拦截；调用方仍只按任务标题执行，并以刷新后的 QQ 等级面板判断完成状态。框架会短时复用同一账号的 QQ 音乐登录 Cookie，并在上游返回 `100050` 时进入 10 分钟冷却，避免刷新或重复点击继续触发换码限流。
