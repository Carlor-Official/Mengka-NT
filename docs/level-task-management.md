...
本次待发布的[小游戏擂台任务扩展](arena-level-task.md)继续使用原等级 API。task80 改为 Go 直接协议上报方块冲刺 v6 的固定 11 分，不运行游戏、不需要外部 worker 安装。11 是已观察成功样本，不是已证实最低分。能力、每日尝试和固定提示字段不变；当日禁用补挂不禁用次日自动计划设置。只有独立 QQ 刷新确认完成才算成功。2026-09-15 02:37，2082083 已在 Linux amd64 测试站通过一次标准 API 的 Go 直接模式验收，依据本次独立证据，不沿用旧 worker 整局结论；尚无新版本或生产发布。升级时保留 journal，见[升级说明](arena-direct-score-upgrade.md)。
