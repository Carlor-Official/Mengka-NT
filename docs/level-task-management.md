...
此能力从 v2.0.9 起提供。v2.0.8 及更早安装包不包含这些管理接口；请先将框架前后端及 SDK 升级到 v2.0.9，再接入插件。
...
覆盖真实本地 WS 认证与 HTTP 的双向设置/缓存一致性、离线读取与实时刷新失败、平台隔离、只读执行日期、付费/完成/协议不可执行过滤、执行后刷新失败不重放、框架本地时间与跨日调度，以及正反向 Node SDK 参数和响应。未调用真实 QQ 账号任务；用户插件本身的适配与计划迁移仍由插件开发任务完成。
