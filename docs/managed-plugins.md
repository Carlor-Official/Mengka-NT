...
2. 安装任务保存在服务器，关闭页面不会取消。进度包含下载、解压、部署和启动；无已知总大小时显示下载量。
...
框架系统服务需以 root 运行，插件进程仍使用独立低权限身份及沙箱。普通框架与分离式 WebSocket 插件不要求安装托管依赖。

v2.1.9 新增环境兼容迁移：

- 安装、启动或恢复托管插件时，自动检查 `bubblewrap`、`util-linux`、`useradd` 和运行命令。缺失或 bubblewrap 功能不足时，通过受信任的系统软件源安装或更新依赖，随后验证命名空间、seccomp 和权限切换。
- 自动准备支持 apt-get、dnf、yum、apk。多个插件共用准备流程，最长 8 分钟；失败短暂退避 1 分钟，避免重复冲击软件源。插件卡片显示准备进度；取消任务或关闭框架可取消等待。
- 框架升级与插件环境准备完全分离。缺少依赖、服务用户不符合条件或内核不支持隔离，都不会阻止框架升级，也不会等待系统软件包安装。新程序启动后在后台准备环境并恢复插件，业务数据与配置保留。更新包校验、文件替换失败回滚仍正常执行。
- 首次运行此兼容版本，会识别已安装且明确因缺少 Linux 隔离依赖或框架运行用户不符而失败的旧插件，并尝试恢复，保留数据及配置。主动停止、卸载、授权失败、包损坏以及无法明确识别的失败不会自动启动。失败原因不再被重启覆盖成通用中断提示。依赖准备失败或准备期间框架重启会保留启动意图，下次框架启动继续恢复；用户主动取消、停止或卸载会清除恢复意图。环境仍未就绪时，后台按 1、2、5、10、30 分钟退避重试，之后每 30 分钟尝试一次；准备成功后自动启动插件，无需再次重启框架。插件卡片显示重试提示，“更多操作 → 停止自动恢复”可取消。

旧版本及此兼容版本的在线更新都不以隔离环境为升级条件；新框架在启动恢复插件时执行依赖准备。使用非 root 的宝塔项目或 systemd 服务，仍需管理员调整框架的运行用户并重启。软件源不可达、软件源版本过旧或宿主机内核/容器禁止隔离时，按提示修复后重试启动；业务数据无需重装。自动准备不会修改系统服务用户、宿主机内核策略或发行版软件源。

Debian / Ubuntu 手工准备命令：

```sh
sudo apt-get update
sudo apt-get install bubblewrap util-linux passwd coreutils
```

软件包存在不代表隔离可用，以框架实际探测结果为准。
