...
连接文件始终包含 `transport`、`instance_id`、`data_directory` 和 `allowed_directories`；旧版 WebSocket 传输另含 `websocket_url` 与 `token_file`。插件每次启动重新读取，不得把令牌发给浏览器、写入日志或固定保存。原生插件格式、Schema 配置和 SDK 见[原生插件协议](native-plugins.md)。
