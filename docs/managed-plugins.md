...
任意已登录官网用户可从 [发布插件](https://mknt.rezcwl.com/uc/plugin-publish) 提交资料，无需开发者身份。只提供一体化部署：填写名称、唯一 ID、版本、简介、Logo、最低框架版本、更新日志及各系统下载地址，选择安装文件，补充接入模式、监听端口和 Web 后台信息。
...
## Releases 自动同步（待下一版本发布）

开发者可在“自动更新源”填写 GitHub 或 Gitee 的仓库 Releases 地址，例如 `https://github.com/Carlor-Official/Bilibili-Utility-Suite/releases`。点击“识别最新版本”后确认各系统/架构安装文件，更新日志可在表单中预览。首次上架及更新源、名称、Logo、简介、运行配置等主要资料变更仍须审核。

审核通过后，官网每 24 小时检查一次正式发行版。开发者在自己的插件列表点击“同步更新”可立即检查，跳过解析缓存；后台与手动任务共用同一流程。资料审核中或已下架的插件不执行自动更新。

只有版本号、更新日志和已审核平台的安装文件允许免审同步。系统按已选择的附件名匹配后续版本，允许版本号和内容哈希后缀变化，不混选不同平台或普通包/托管包。缺包、多义、版本不一致、网络失败时保留旧版，列表显示同步结果。平台、运行配置或仓库变更请编辑资料重新提交审核。自动同步沿用签名目录，并保存上一版签名记录。

框架市场会在已安装卡片显示“在线更新 vX.Y.Z”。点击可查看更新日志和版本差异，确认后才下载、校验并切换程序。升级保留插件实例、业务配置和数据库，启动失败尝试恢复旧程序。官网同步不自动升级用户正在运行的插件。普通安装包沿用 schema 2 的下载和包验证契约；schema 1 继续校验固定 SHA-256 与大小。

已有插件继续保持原行为；要启用此功能，请在发布表单绑定自动更新源并提交一次审核。SDK 的 WS API 未改变。
