...
2. 搜索中文名称或 action，按分类选择 API。当前目录与 SDK 的 233 个 action 对齐；QQ 宠物 30 项接口已按星空花海贡献附件重新定义，使用新对象参数。
...
受控 API（当前仅 send_packet）要求已授权的托管插件运行会话。调试页面不能代替该身份，因此禁止从此页面直接发送；普通 API 保持正常调试。用户无需在头像菜单绑定 Key，可在“插件 API 授权”查看本机托管会话，详细步骤见 [插件 API 授权](plugin-api-authorization.md)。
...
调试请求上限 1 MiB；响应总量上限 10 MiB。流式分片在本次请求结束后统一展示；超过预览上限的大文件传输应使用 SDK。未知接口、服务端参数错误、QQ 协议限制、插件权限拒绝均保留原执行链路的失败信息。

通道仅连接本机已配置、已运行的正向服务，使用该服务当前令牌进行普通 WS 认证，不接受任意远程 URL，不绕过插件 dispatcher、插件权限检查，也不恢复已删除的 action / 参数。接口及参数以当前 SDK 文档为准。
