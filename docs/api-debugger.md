...
2. 搜索中文名称或 action，按分类选择 API。当前目录与 SDK 的 234 个 action 对齐；QQ 宠物 30 项接口已按星空花海贡献附件重新定义，使用新对象参数。
...
`send_packet` 与其他普通 API 一样，可通过已启动的正向 WS 服务进行调试；调试入口保留框架管理员登录要求，无需插件白名单或专属 Key。
