...
元宝登录返回安全校验时，框架管理页面会展示经过校验的 `https://yuanbao.tencent.com/e/login` 官方入口，同时通过私聊消息把同一入口发送给当前 QQ 自身。消息收件人由框架固定为触发任务的账号，前端和插件不能指定或改写。用户可在管理页面、独立窗口或 QQ“私聊自己”中完成腾讯验证，再回到等级加速页面点击“已完成验证，继续任务”。接口数据中的 `qq_message_sent` 表示私聊消息是否发送成功；即使发送失败，前端入口仍可使用。框架不会代理验证码、把独立验证代码写入配置或数据库，或后台循环生成新验证。插件调用仍会收到需要安全验证的失败结果，应引导管理员完成一次人工验证，不要自动重放。
