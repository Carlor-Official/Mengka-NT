...
擂台扩展待发布：task80“创建小游戏擂台并取得成绩”由 Go 直接协议上报方块冲刺 v6 的固定 11 分，不运行游戏，不依赖外部 worker 安装；没有分数、游戏或计时参数。11 是已观察成功样本，不是已证实最低分。当天任何持久尝试均不可重放，结果未知也不重试；仅最终 `wx.login` 确证 `authorization_required` 才提示“微信未授权登录”。只有同房本人分数读回、退出协议确认及独立 `0x916e → 0x9172` 确认完成才算成功。SDK 保持 5 分钟，框架等级请求总预算 4 分 45 秒。新模式已在 Linux amd64 测试站通过 2082083 的标准任务验收，尚无生产发布，见[擂台契约](../arena-level-task.md)。
