...
擂台扩展待发布：task80“创建小游戏擂台并取得成绩”当前仅方块冲刺 v6，没有分数、游戏或计时参数。当日任何持久尝试均不可重放，结果未知也不重试；仅确证授权状态才提示“微信未授权登录”。SDK 保持 5 分钟，框架等级请求上限 4 分 45 秒、worker 4 分 15 秒。见[擂台契约](../arena-level-task.md)。
