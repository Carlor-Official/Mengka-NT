...
const apiDefs = {
  ...
  start_pet_activity: { wait: true, build: (self_id, activity, option_name = '', friend_id = 0, sub_event_type = 0) => ({ self_id, activity, option_name, friend_id, sub_event_type }) },
  ...
  get_pet_fatigue_status: { wait: true, build: (self_id) => ({ self_id }) },
  ...
  // 1.9.7 系统管理专用接口。权限包也会提升已授权 Bot 管理 action 的节点作用域。
