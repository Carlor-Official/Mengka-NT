...
v2.1.5 的 Linux 托管插件要求框架以 root 系统服务运行，并具备 `bubblewrap`（`/usr/bin/bwrap`）、`util-linux`（`setpriv`）和 `useradd`。缺少依赖时插件会显示运行环境不可用；普通框架运行不要求这些托管条件。

Debian / Ubuntu 可安装：

```sh
sudo apt-get install bubblewrap util-linux passwd
```

其他发行版安装提供上述命令的对应软件包。升级前确认服务运行账号与依赖，随后从框架重新启动插件。
