...
对指定当前活动提交鼓励，不补造积分或奖励。

- action：`encourage_pet_activity`
- 支持协议：Android。
...
```js
const result = await api.encourage_pet_activity({
  "self_id": 123456,
  "client_type": "android",
...
submitted、effect_verified、pet_id、messages[]、toast；服务端提供时含 credit。
