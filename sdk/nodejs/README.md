...
task83“来元宝P图一次”使用现有 `execute_level_tasks` / `execute_level_task_selection`，SDK 方法、参数和 5 分钟等待上限不变。插件不要采集或传入元宝 token、模型名、图片或安全验证码；框架从目标 QQ 当前登录态完成短期授权、临时上传和官方智能 P 图。仅元宝明确拒绝一种图片路由时才会尝试下一种，且必须收到真实图片结果事件；普通文本或结果不明确时不重放。元宝要求安全校验时，框架会在等级加速页面展示腾讯官方入口，并把同一入口私聊发送给当前 QQ 自身；插件不能指定消息收件人，应引导管理员完成一次人工验证，不要自动重放或代理验证码。结构化错误的 `qq_message_sent` 表示私聊是否成功，失败时仍可使用管理页面入口。验证后的元宝会话仅在框架进程内短期复用。目标 QQ 需先在官方元宝 App 登录注册并激活图片能力，执行后的完成状态以 QQ 面板刷新结果为准。详见[执行 QQ 等级加速任务](../../docs/api/execute_level_tasks.md)。
