...
当前 SDK 版本：**v2.4.1**。新增原生 IPC，继续支持正反向 WS、本地插件导入与托管管理员初始化；小游戏擂台任务继续使用现有等级 API，见下方升级说明。

## v2.4.1：原生插件协议 v1

- 新增 `native-plugin.js`，使用 stdin/stdout JSON 行协议接收生命周期、配置和事件并调用框架 Action。
- 原生安装包最低框架版本为 v2.4.1；已有正向、反向 WebSocket 插件保持兼容。
- 敏感配置由框架加密保存，SDK 仅接收运行时快照，禁止写入日志。
...
官方用户系统原生 Linux AMD64 外发包（版本 `2.0.17`，最低框架 `2.4.1`）随 [萌卡 NT v2.4.1 Release](https://github.com/Carlor-Official/Mengka-NT/releases/tag/v2.4.1) 发布。下载后在框架「插件 → 插件导入」上传，并使用同一 Release 的 `linux-amd64-native.json` 与 `SHA256SUMS.txt` 完成校验。
...
- 本地托管插件由框架启动并管理生命周期；
- 保留正向与反向 WebSocket，三种部署方式共享同一套 action、事件和权限语义；
...
- `execute_level_tasks` 的擂台任务在创建成功后允许本人列表和房间详情短暂延迟，框架只重读确认，不会重复创建。
- 失败时可在账号“近期日志”查看固定阶段和脱敏 QQ 数字错误码；任务保持待完成并返回原因，可由用户再次执行。
...
`plugin-connection.js` 提供统一的 `start/stop` 入口，支持手动正向和手动反向连接；本地托管插件由框架注入运行时配置。完整说明见[双模式接入指南](docs/dual-mode-sdk.md)，可运行 Demo 位于 `examples/dual-mode/server.mjs`。示例不含任何实际令牌、插件授权或支付密钥。
...
[表情回应事件](docs/message-reactions.md)：正向连接支持显式声明 group_event，并继续按监听器推导权限。[名片与头像](docs/api/get_summary_card.md)：新增 avatar_only 选项和 avatar_url，需升级框架后使用。

## v2.1.11：洗护购买业务结果透传

`buy_pet_bath_item` 直接返回商城业务 `result` 与 `order_id`，非零结果码或空订单号不再转成调用异常。插件需自行解释业务结果；`submitted` 只表示已提交，`effect_verified` 仍为 `false`。网络、解析和 OIDB 外层错误继续报错，不自动重试购买。

## v2.1.10：独立低权限插件进程

Linux 托管插件改用各自专属的普通用户运行，不再依赖 bubblewrap 或命名空间。框架自动复制迁移旧默认目录，保留原数据并支持失败恢复；本地导入包的安装校验、服务授权和管理入口继续使用现有流程。框架服务仍需 root，不会自动提权；宿主公共文件仍可能被插件访问。升级备份与回滚要求见[托管插件说明](../../docs/managed-plugins.md)。本版不改变公开 action、参数、返回结构或 SDK 调用方法。

## v2.1.9：隔离环境自动恢复与洗护购买修复

框架升级不再等待插件隔离环境准备；升级后后台补全依赖并自动重试恢复，保留插件数据与启动意图。Linux 托管插件仍需 root 服务及内核隔离支持。buy_pet_bath_item 的 item_id 继续传目录返回的数字字符串（如 "1"），框架修正上游商品编号编码；count 仅校验大于 0，不预查目录限制。

## v2.1.8：洗护购买与托管运行

buy_pet_bath_item 的数量业务校验仅要求 count > 0，不再预查目录上下限或步进。框架修复在线更新与托管插件重启生命周期，并提供官方用户系统的 /user/ 公共门户配套；公开 action 和 SDK 参数不变。

## v2.1.7：PK 状态与结算

get_pet_pk_status 成功时返回明确的 finished 布尔值；settle_pet_pk 直接提交一次结算，不强制查询状态。请在结算后复查数值，结果未知时不要重复提交。
...
新增独立服务端助手 `managed-connection.js`：`loadManagedConnection()` 读取独立部署的 WS 地址及令牌，`loadManagedStorage()` 读取存储与允许目录。详见[一体化插件接入](../../docs/managed-plugins.md)。公开 WS action 数量、名称和参数顺序保持兼容。
...
当前文档与框架 v2.4.1 发布候选对齐，更新于 2026-09-18。会员签到与电脑在线沿用共享等级任务 API；好友备注写入后回读确认，必须显式传入字符串。初始化 HTTP 客户端需遵守[协议确认契约](../../docs/initialization-agreement.md)，前后端必须一起更新。
