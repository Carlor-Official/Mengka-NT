...
SDK 1.0.4 的方法列表和参数字段已与当前服务端注册的插件 API 对齐。账号管理及登录方法仍以 QQ 号作为第一个参数，SDK 会按服务端协议发送 `self_id`。
...
1.0.3 中列出但当前服务端未注册的空间活动、QQ 阅读/农场、音乐加速和原始 OIDB 方法已从 SDK 移除，避免调用后才收到“未知 action”。
