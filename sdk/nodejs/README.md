...
当前开发版 SDK 已同步 92 个公开 action，包含首批 OneBot/NapCat 系统基础接口以及 `send_private_msg`、`send_msg` 消息兼容接口。插件市场服务的可调用 API 和可订阅事件以开发者在萌卡 NT 官网勾选、审核并随安装冻结的权限快照为准；SDK 中存在某个方法不代表插件已经获得该权限。
