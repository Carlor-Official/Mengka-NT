...
当前 SDK 版本：**v2.4.0**。支持正向 WS、反向 WS 与框架本地托管；插件成品由用户在框架「插件导入」中上传，不再依赖官网插件市场。
...
当前文档与框架 v2.3.1 对齐，更新于 2026-09-15。会员签到与电脑在线沿用共享等级任务 API；好友备注写入后回读确认，必须显式传入字符串。初始化 HTTP 客户端需遵守[协议确认契约](../../docs/initialization-agreement.md)，前后端必须一起更新。
