...
当前 1.9.7 SDK 保留 218 个 action 名称，对应 216 项能力与 2 个兼容别名。`system_management` 是权限覆盖层：完整权限包由 22 个框架管理专用 action 和 20 个复用现有处理器的 Bot 管理 action 组成。QQ 宠物 API 仍为普通公开接口，专属 Key 仅用于 `send_packet`。管理能力同时受服务总开关与允许清单校验，不会自动授予普通插件。

系统管理服务可使用 `get_plugin_context` 检查授权上下文，以 `get_bot_list({ all_nodes: true })` 读取全节点账号，并通过 `add_account({...})` / `update_account({...})` 的对象参数指定 `node_id` 与 `client_type`。`generate_device_profile`、`stop_account_login` 分别是 `create_device_profile`、`offline_account` 的兼容别名；新插件应使用标准名称。完整权限清单、调用示例和安全边界见 [Node.js SDK 文档](sdk/nodejs/README.md#197-系统管理接口) 与 [官网 API 文档](https://mknt.net/api/)。
