...
框架适用于 Windows 与 Linux，可同时管理多个 QQ 账号，并为每个账号保留独立的协议、设备、节点和登录状态。首次启动会进入浏览器中的两步初始化页面，阅读使用协议后创建仅保存在当前框架中的本地管理员；初始化完成后安装入口会自动锁定。
...
当前版本：**2.3.9**。查看[本版更新说明](release-notes-v2.3.9.md)。

v2.3.9 将“来元宝P图一次”恢复为“暂不支持”，并停止该任务的自动执行。
...
2. 阅读并同意萌卡 NT 使用协议；
3. 填写 Web 服务监听端口；
4. 输入本地管理员账号、密码并再次确认密码；
5. 完成初始化并使用刚创建的本地管理员进入管理后台。

初始化完成后安装入口会锁定，再次访问安装地址会返回 404。管理员账号和 bcrypt 密码摘要只保存在当前框架的本地数据库中，不依赖萌卡 NT 官网用户系统。
...
不要将插件令牌直接写入源码或提交到公开仓库。完整接口和登录流程见 [萌卡 NT 开发文档](https://mknt.rezcwl.com/)。
...
- 插件开发：[Node.js SDK](sdk/nodejs) · [WebUI SDK](sdk/plugin-web) · [开发文档](https://mknt.rezcwl.com/api/)
