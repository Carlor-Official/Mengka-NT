...
v2.3.1 修复小游戏擂台创建后的延迟回读误判，并补充分阶段、脱敏的账号日志。擂台失败会返回原因并保持待完成，可再次执行；只有 QQ 确认完成才显示已完成。单轮创建请求仍只发送一次，分数继续使用 1–99 随机整数。见[本版更新说明](release-notes-v2.3.1.md)与[故障诊断说明](docs/arena-create-diagnostics.md)。
...
2. 阅读并同意萌卡 NT 使用协议；
3. 填写 Web 服务监听地址和端口；
4. 前往[萌卡 NT 官网个人令牌页面](https://mknt.rezcwl.com/uc/profile?tab=pat)创建并填写个人令牌；
