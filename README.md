...
最新正式版本：**2.4.2**。当前开发版本：**2.4.2**，查看[本版更新说明](release-notes-v2.4.2.md)。

v2.4.0 将管理员认证改为本地账号，并用用户手动上传的“插件导入”替代官网插件市场。

等级加速面板已从源头移除会员推广位及日常、额外分类的付费任务；仪表盘加速数值与进度直接采用 QQ 等级面板返回的数据，会员签到仍单独保留。

插件导入支持上传成品包、运行管理、日志、数据目录和网页管理。v2.4.1 新增原生插件配置 Schema，由框架生成设置页、加密保存敏感字段并通过 IPC 热更新。v2.4.2 新增 Linux QQ 3.2.33 可选协议，并修复部分账号扫码授权提示版本过低的问题。Linux 账号支持同 QQ 安卓授权免扫登录，等级任务“电脑QQ在线”复用同一流程。
