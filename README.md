...
萌卡 NT（Mengka NT）是一套面向 QQ NT 协议的机器人框架。它将账号、设备指纹、连接节点、插件服务和运行状态集中到 Web 管理界面，并通过原生 IPC 插件、插件 WebSocket 与 Node.js SDK 提供消息、联系人、群管理及扩展业务能力。

框架适用于 Windows 与 Linux，可同时管理多个 QQ 账号，并为每个账号保留独立的协议、设备、节点和登录状态。首次启动会进入浏览器中的两步初始化页面，阅读使用协议后直接设置本地管理员账号和密码；初始化完成后安装入口会自动锁定。

> 本仓库是萌卡 NT 的官方版本发布与插件 SDK 文档入口，不提供框架核心业务源码，也不包含运行配置、账号数据、数据库或密钥。

官网：[萌卡 NT](https://mknt.net/) · [官网与部署文档](https://mknt.net/guide/)

最新正式版本：**2.4.4**。当前开发版本：**2.4.4**，查看[本版更新说明](release-notes-v2.4.4.md)。

v2.4.0 将管理员认证改为本地账号，并用用户手动上传的“插件导入”替代官网插件市场。

等级加速面板已从源头移除会员推广位及日常、额外分类的付费任务；仪表盘加速数值与进度直接采用 QQ 等级面板返回的数据，会员签到仍单独保留。

插件导入支持上传成品包、运行管理、日志、数据目录和网页管理。v2.4.1 新增原生插件配置 Schema，由框架生成设置页、加密保存敏感字段并通过 IPC 热更新。v2.4.3 的 Linux QQ 仅保留 3.2.32，并统一扫码、免扫与等级任务“电脑QQ在线”流程；v2.4.4 以金币兑换接口的业务结果为准，不再用部分账号不准确的 QQ 音乐余额查询结果提前拦截，同时复用短时音乐登录 Cookie 并抑制重复换码。
...
| 平台 | 外发包 | 启动文件 | 架构 |
| --- | --- | --- | --- |
| Windows | `mengka-nt-*-windows-amd64.zip` | `mengka-nt.exe` | AMD64 / x86_64 |
| Linux | `mengka-nt-*-linux-amd64.tar.gz` | `mengka-nt` | AMD64 / x86_64 |
| Linux | `mengka-nt-*-linux-arm64.tar.gz` | `mengka-nt` | ARM64 / AArch64 |
...
初始化完成后安装入口会锁定。管理员凭据只保存在当前框架实例中，不与官网账号系统绑定。
...
仓库在 [`sdk/nodejs`](sdk/nodejs) 提供不带版本子目录的 Node.js 插件 SDK，支持原生 IPC 与正反向 WebSocket、接收机器人事件并调用框架 Action；[`sdk/plugin-web`](sdk/plugin-web) 提供插件网页后台接入 SDK。原生安装包格式见[原生插件协议 v1](docs/native-plugins.md)。
...
不要将插件令牌直接写入源码或提交到公开仓库。完整接口和登录流程见 [萌卡 NT 开发文档](https://mknt.net/)。

插件提供网页后台时，请参考 [WebUI SDK 接入说明](sdk/plugin-web/README.md)，使用框架分配的本地端口、挂载路径与令牌文件，不要把管理令牌写入前端代码或 URL。原生插件必须在包内声明 Action 与事件权限；外部 WebSocket 服务继续使用现有服务认证与事件订阅契约。
...
v2.0.9 新增六个共享等级任务管理接口，插件与框架网页共用任务计划、缓存及执行逻辑。插件接入后需停用自己的等级任务定时器，账号归属与用户权限仍由插件校验。见[接入说明](docs/level-task-management.md)。

## 闪照 API

v2.2.1 新增 [get_flash_image](docs/api/get_flash_image.md)，插件可通过消息 ID 获取闪照原图地址和图片信息。
